                ...
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token0.derivedAVAX)
                      ? formattedNum(position?.fees.sum / (parseFloat(position.pair.token0.derivedAVAX) * avaxPrice) / 2, false, true)
                      : 0}{' '}
                  </TYPE.small>
                  <FormattedName text={position.pair.token0.symbol} maxCharacters={below740 ? 10 : 18} margin={true} fontSize={'11px'} />
                </RowFixed>
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token1.derivedAVAX)
                      ? formattedNum(position?.fees.sum / (parseFloat(position.pair.token1.derivedAVAX) * avaxPrice) / 2, false, true)
                      : 0}{' '}
                  </TYPE.small>
                  <FormattedName text={position.pair.token1.symbol} maxCharacters={below740 ? 10 : 18} margin={true} fontSize={'11px'} />
                </RowFixed>
      ...
      <DashGrid center={true} style={{ height: '32px', padding: 0 }}>
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              {below740 ? 'Fees' : 'Total Fees Earned'} {sortedColumn === SORT_FIELD.UNISWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
