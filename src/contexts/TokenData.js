...
const getTokenData = async (address, avaxPrice, avaxPriceOld, chainId) => {
  ...
  try {
    ...
    const priceChangeUSD = getPercentChange(data?.derivedAVAX * avaxPrice, parseFloat(oneDayData?.derivedAVAX ?? 0) * avaxPriceOld)
...
const getIntervalTokenData = async (tokenAddress, startTime, interval = 3600, latestBlock, chainId) => {
  ...
  try {
    ...
    let timestampedPrices = await getAvaxPriceAtTimestamp(startTime)

    // Go through AVAX USD prices and assign to original values array
    let index = 0
    for (var brow in result) {
      let startIndex = 0,
        endIndex = timestampedPrices.length - 1,
        found = -1
      ...
      if (timestamp) {
        ...
        if (found >= 0) {
          values[index].priceUSD = timestampedPrices[found][1] * values[index].derivedAVAX
          index += 1
        }
      }
    }
